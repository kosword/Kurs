...
QUrl Dropbox::apiToUrl(Dropbox::Api api)
{
    ...
    case Dropbox::FILESPUT:
        return QUrl(
            QString(
            "https://api-content.dropbox.com/%1/files_put/sandbox"
            ).arg(apiVersion)
            );
        break;
    ...
    case Dropbox::THUMBNAILS:
        return QUrl(
            QString(
            "https://api-content.dropbox.com/%1/thumbnails/sandbox"
            ).arg(apiVersion)
            );
        break;
    }
...
QString Dropbox::metaDataPathFromUrl(QUrl url)
{
    ...
    metaDataPath = metaDataPath.remove(
        QString("/%1/metadata/sandbox").arg(apiVersion)
        );
...
QString Dropbox::filePathFromUrl(QUrl url)
{
    ...
    filePath = filePath.remove(
        QString("/%1/files/sandbox").arg(apiVersion)
        );
